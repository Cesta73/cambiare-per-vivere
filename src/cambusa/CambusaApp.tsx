...
function RemoveDialog({ product, onClose, onDone }: { product: PantryProduct; onClose: () => void; onDone: () => Promise<void> }) {
  ...
  return createPortal(<div className="dialog-backdrop"><div className="dialog small"><div className="dialog-head"><div><span>Scarico manuale</span><h2>{product.name}</h2></div><button className="icon-button" onClick={onClose} aria-label="Chiudi"><X /></button></div><label>Quantità da scaricare<input type="number" value={quantity} min="0.001" max={product.total_quantity} step="0.001" onChange={(event) => setQuantity(Number(event.target.value))} /></label><p>Disponibili: {product.total_quantity} {product.default_unit}</p><button className="primary-button" disabled={busy || quantity <= 0 || quantity > product.total_quantity} onClick={() => void remove()}>Conferma scarico</button></div></div>, document.body);
}
